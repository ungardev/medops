  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-white/50 hover:text-white px-5 py-2.5 bg-white/5 border border-white/15 rounded-xl transition-all">
            <ArrowLeftIcon className="w-5 h-5" /> Volver
          </button>
        }
      />
      
      <div className="flex items-center gap-5 bg-white/5 border border-white/15 p-5 rounded-xl">
      ...
      <div className="flex border-b border-white/15">
        ...
        >
          <ClockIcon className="w-5 h-5" /> Horarios Semanales
        </button>
      </div>
      
      <div className="bg-white/5 border border-white/15 p-6 rounded-xl">
        {activeTab === 'info' && (
          <div className="space-y-4">
            <h2 className="text-lg font-medium text-white/80">Detalles del Servicio</h2>
            <p className="text-white/50 text-sm">{service.description || 'Sin descripción.'}</p>
          </div>
        )}
        ...
        {activeTab === 'schedules' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-6 gap-4 p-5 bg-white/5 rounded-xl border border-white/10">
              <div className="md:col-span-1">
                <label className="block text-xs text-white/40 uppercase mb-2">Día</label>
                <select 
              ...
              <div className="md:col-span-1">
                <label className="block text-xs text-white/40 uppercase mb-2">Inicio</label>
                <input 
                  type="time" 
              ...
              <div className="md:col-span-1">
                <label className="block text-xs text-white/40 uppercase mb-2">Fin</label>
                <input 
                  type="time" 
              ...
              <div className="md:col-span-1">
                <label className="block text-xs text-white/40 uppercase mb-2">Duración (min)</label>
                <input 
                  type="number" 
            ...
            <div className="grid grid-cols-7 gap-3 mt-6">
              {DIAS_SEMANA.map((dia) => {
                ...
                return (
                  <div key={dia.value} className="border border-white/15 rounded-xl bg-white/5 min-h-[220px] flex flex-col">
                    <div className="p-4 text-center border-b border-white/10 bg-white/5 rounded-t-xl">
                      <span className="text-sm font-medium text-white/60">{dia.label}</span>
                    </div>
                    
                    <div className="p-4 flex-1 space-y-3">
                      {daySchedules.length === 0 ? (
                        <div className="h-full flex items-center justify-center">
                          <span className="text-xs text-white/20 italic">Sin horario</span>
                        </div>
                      ) : (
                        daySchedules.map((schedule) => (
                          <div 
                            key={schedule.id} 
                            className="group relative bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 hover:bg-emerald-500/15 transition-colors"
                          >
                            <div className="flex justify-between items-start gap-2">
                              ...
                              <button 
                                ...
                                className="opacity-0 group-hover:opacity-100 text-red-400/60 hover:text-red-400 transition-opacity p-1.5 rounded-lg hover:bg-red-500/10"
                              >
