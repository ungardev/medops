...
export default function Patients() {
  ...
  const handleDelete = (p: Patient) => {
    if (window.confirm(`¿Estás seguro de eliminar al paciente ${p.full_name}? Esta acción no se puede deshacer.`)) {
      deletePatient(p.id, {
        onSuccess: () => {
  ...
  const searchSection = (
    <div className="relative group">
      <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
        <MagnifyingGlassIcon className={`w-5 h-5 transition-colors ${isSearching ? 'text-emerald-400 animate-pulse' : 'text-white/30'}`} />
      </div>
      <input
        type="text"
        value={query}
