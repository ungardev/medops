...
import { usePersonalHistory } from "../../../hooks/patients/usePersonalHistory";
import PersonalHistoryModal from "./PersonalHistoryModal";
...
export default function PersonalHistorySection({ items, patientId, onRefresh }: Props) {
  const { query, create, update, remove } = usePersonalHistory(patientId);
    ...
    if (editingItem) {
      update.mutate(
        { ...editingItem, ...data },
        {
          ...
          onError: () =>
            setLocalError("No se pudo actualizar el antecedente. Intenta nuevamente."),
        }
      );
    } else {
      create.mutate(
        { ...data, patient: patientId },
        {
          ...
          onError: () =>
            setLocalError("No se pudo crear el antecedente. Intenta nuevamente."),
        }
      );
    }
  };
  ...
  const handleDelete = (id: number) => {
    ...
    remove.mutate(id, {
      ...
      onError: () =>
        setLocalError("No se pudo eliminar el antecedente. Intenta nuevamente."),
    });
  };
