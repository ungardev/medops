...
export default function ClinicalProfileSection({
      ...
      <div className={`border-b border-white/10 last:border-0 transition-all ${isExpanded ? 'bg-white/[0.02]' : ''}`}>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className={`text-sm font-medium ${isExpanded ? 'text-white/80' : 'text-white/40'}`}>
              {title}
            </span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <ChevronDownIcon className={`w-5 h-5 transition-transform duration-300 ${isExpanded ? 'rotate-180 text-emerald-400' : 'text-white/30'}`} />
          </div>
        </button>
        {isExpanded && (
          <div className="px-6 pb-6 pt-3 animate-in fade-in slide-in-from-top-2">
            <div className="space-y-3">
              {items.length === 0 ? (
                <div className="text-sm text-white/30 py-5 border border-dashed border-white/10 rounded-xl text-center">
                  Sin registros
                </div>
              ) : (
                items.map((item) => (
                  <div key={item.id} className="group flex items-center justify-between p-5 border border-white/10 bg-white/5 hover:border-white/20 transition-all rounded-xl">
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-3">
                        ...
                        {item.severity && (
                          <span className={`text-xs px-3 py-1 border rounded-lg font-medium ${
                    ...
                    {!readOnly && (
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                          ...
                          className="p-2.5 text-white/30 hover:text-emerald-400 transition-colors rounded-xl hover:bg-white/5"
                        >
                          <PencilIcon className="w-5 h-5" />
                        </button>
                        <button 
                          onClick={() => handleDelete(item, type)}
                          className="p-2.5 text-white/30 hover:text-red-400 transition-colors rounded-xl hover:bg-red-500/10"
                        >
              ...
              {!readOnly && (
                <button
                  ...
                  className="w-full py-3.5 border border-dashed border-white/10 hover:border-emerald-500/30 hover:bg-emerald-500/5 transition-all group flex justify-center items-center gap-2 rounded-xl"
                >
                  <PlusIcon className="w-5 h-5 text-white/30 group-hover:text-emerald-400" />
                  <span className="text-sm font-medium text-white/30 group-hover:text-emerald-400">
                    Agregar registro
                  </span>
                </button>
              )}
  ...
  return (
    <div className="bg-white/5 border border-white/15 rounded-xl overflow-hidden">
      <div className="bg-white/5 px-6 py-4 border-b border-white/15">
