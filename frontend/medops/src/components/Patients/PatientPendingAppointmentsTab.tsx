...
export default function PatientPendingAppointmentsTab({ patient }: PatientTabProps) {
  ...
  if (isLoading) return <p className="text-sm text-gray-600 dark:text-gray-400">Cargando próximas citas...</p>;
  if (error) return <p className="text-sm text-red-600 dark:text-red-400">Error: {(error as Error).message}</p>;
  if (isEmpty) return <p className="text-sm text-gray-500 dark:text-gray-400">No tiene citas pendientes</p>;

  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm p-4 bg-white dark:bg-gray-900">
      <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-4">Citas pendientes</h3>
