  ...
  return (
    <div className="space-y-4">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-white/5 border-b border-white/15">
              ...
              <th className="px-5 py-4 text-xs font-medium text-white/50 uppercase tracking-wider">Tipo</th>
              <th className="px-5 py-4 text-xs font-medium text-white/50 uppercase tracking-wider">Estado</th>
              <th className="px-5 py-4 text-xs font-medium text-white/50 uppercase tracking-wider">Notas</th>
            </tr>
          </thead>
            ...
            {appointments?.map((a: Appointment) => {
              ...
              return (
                <tr key={a.id} className="hover:bg-white/5 transition-colors group">
                  ...
                  <td className="px-5 py-4">
                    <span className="text-xs bg-emerald-500/10 text-emerald-400 px-3 py-1.5 border border-emerald-500/20 uppercase font-medium rounded-lg">
                      {a.appointment_type === 'general' ? 'General' : a.appointment_type}
                    </span>
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full animate-pulse ${isActive ? 'bg-emerald-400' : 'bg-amber-400'}`} />
                  ...
                  <td className="px-5 py-4">
                    <p className="text-xs text-white/40 max-w-xs truncate group-hover:whitespace-normal">
                      {a.notes || "—"}
                    </p>
                  </td>
                </tr>
      ...
      <div className="sm:hidden space-y-3">
        {appointments?.map((a: Appointment) => (
          <div key={a.id} className="p-5 bg-white/5 border border-white/15 rounded-xl relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-4">
              <div className="flex flex-col">
                ...
                <span className="text-sm text-white/70 font-medium">{a.appointment_type === 'general' ? 'General' : a.appointment_type}</span>
              </div>
            ...
            {a.notes && (
              <div className="mt-4 p-3 bg-white/5 rounded-xl flex gap-2">
                <ChatBubbleBottomCenterTextIcon className="w-4 h-4 text-white/30 flex-shrink-0" />
                <p className="text-xs text-white/40 italic leading-relaxed">{a.notes}</p>
              </div>
            )}
          </div>
        ))}
