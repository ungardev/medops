      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-xl bg-[#1a1a1b] border-l border-white/15 z-50 shadow-2xl transform transition-transform duration-300 ease-out overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/15 bg-[#1a1a1b]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-white">Detalle de Hospitalizaci\u00f3n</h2>
              <p className="text-sm text-white/40">Informaci\u00f3n completa del paciente</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
            ...
            <p className="text-base text-white/80 font-medium">{hospitalization.patient_name || "Sin informaci\u00f3n"}</p>
          </div>
            ...
            <div className="space-y-2">
              {(hospitalization as any).hospital && (
                <div>
                  ...
                  <p className="text-sm text-white/70">{(hospitalization as any).hospital}</p>
                </div>
              )}
              ...
              {hospitalization.ward && (
                <div className="flex gap-4">
                  <div>
                    <span className="text-xs text-white/40 uppercase">Sala/Pabell\u00f3n</span>
                    <p className="text-sm text-white/70">{hospitalization.ward}</p>
                  </div>
                  {hospitalization.room_number && (
                    <div>
                      <span className="text-xs text-white/40 uppercase">Habitación</span>
                      <p className="text-[12px] text-white/70">{hospitalization.room_number}</p>
                    </div>
                  )}
                  {hospitalization.bed_number && (
                    <div>
                      <span className="text-xs text-white/40 uppercase">Cama</span>
              ...
              {!hospitalization.ward && (
                <p className="text-sm text-white/40">No hay informaci\u00f3n de ubicaci\u00f3n</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {hospitalization.admission_date && (
                <div>
                  <span className="text-xs text-white/40 uppercase">Fecha de Ingreso</span>
              ...
              {(hospitalization as any).expected_discharge_date && (
                <div>
                  <span className="text-xs text-white/40 uppercase">Fecha Estimada de Alta</span>
              ...
              {(hospitalization as any).actual_discharge_date && (
                <div>
                  <span className="text-xs text-white/40 uppercase">Fecha Real de Alta</span>
                  <p className="text-[12px] text-white/70">
          ...
          {(hospitalization as any).attending_doctor_name && (
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
              ...
              <p className="text-base text-white/80">{(hospitalization as any).attending_doctor_name}</p>
            </div>
          )}
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
            ...
            <div className="space-y-2">
              {hospitalization.admission_type && (
                <div>
                  <span className="text-xs text-white/40 uppercase">Tipo de Admisi\u00f3n</span>
                  <p className="text-sm text-white/70 capitalize">{hospitalization.admission_type}</p>
                </div>
              )}
              
              {(hospitalization as any).chief_complaint && (
                <div>
                  <span className="text-xs text-white/40 uppercase">Motivo de Ingreso</span>
              ...
              {(hospitalization as any).clinical_summary && (
                <div>
                  <span className="text-xs text-white/40 uppercase">Resumen Cl\u00ednico</span>
              ...
              <div className="grid grid-cols-2 gap-3">
                {vitalSigns.weight && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-xs text-white/40 uppercase">Peso</span>
                      <p className="text-sm text-white/70">{vitalSigns.weight} kg</p>
                    </div>
                  </div>
                )}
                {vitalSigns.height && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-xs text-white/40 uppercase">Altura</span>
                      <p className="text-sm text-white/70">{vitalSigns.height} cm</p>
                    </div>
                  </div>
                )}
                {bmi && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-xs text-white/40 uppercase">IMC</span>
                      <p className="text-sm text-blue-400 font-medium">{bmi}</p>
                    </div>
                  </div>
                )}
                {vitalSigns.temperature && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-xs text-white/40 uppercase">Temperatura</span>
                      <p className="text-sm text-white/70">{vitalSigns.temperature} °C</p>
                    </div>
                  </div>
                )}
                {vitalSigns.bp_systolic && vitalSigns.bp_diastolic && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-xs text-white/40 uppercase">Presi\u00f3n Arterial</span>
                      <p className="text-sm text-white/70">{vitalSigns.bp_systolic}/{vitalSigns.bp_diastolic} mmHg</p>
                    </div>
                  </div>
                )}
                {vitalSigns.heart_rate && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-xs text-white/40 uppercase">Frecuencia Card\u00edaca</span>
                      <p className="text-sm text-white/70">{vitalSigns.heart_rate} bpm</p>
                    </div>
                  </div>
                )}
                {vitalSigns.respiratory_rate && (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-xs text-white/40 uppercase">Frecuencia Respiratoria</span>
                      <p className="text-sm text-white/70">{vitalSigns.respiratory_rate} /min</p>
                    </div>
                  </div>
                )}
                {vitalSigns.oxygen_saturation && (
                  <div className="flex items-center gap-2">
                    <Activity className="w-4 h-4 text-blue-400" />
                    <div>
                      <span className="text-xs text-white/40 uppercase">Sat. Ox\u00edgeno</span>
                      <p className="text-sm text-white/70">{vitalSigns.oxygen_saturation}%</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          {(hospitalization as any).allergies_at_admission && (
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
              ...
              <p className="text-sm text-white/70 whitespace-pre-wrap">{(hospitalization as any).allergies_at_admission}</p>
            </div>
          )}
          ...
          {(hospitalization as any).daily_notes && (
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
              ...
              <p className="text-sm text-white/70 whitespace-pre-wrap">{(hospitalization as any).daily_notes}</p>
            </div>
          )}
          ...
          {(hospitalization as any).complications && (
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
              ...
              <p className="text-sm text-white/70 whitespace-pre-wrap">{(hospitalization as any).complications}</p>
            </div>
          )}
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
              ...
              <div className="space-y-2">
                {(hospitalization as any).discharge_type && (
                  <div>
                    <span className="text-xs text-white/40 uppercase">Tipo de Alta</span>
                ...
                {(hospitalization as any).discharge_summary && (
                  <div>
                    <span className="text-xs text-white/40 uppercase">Resumen de Alta</span>
                ...
                {(hospitalization as any).discharge_instructions && (
                  <div>
                    <span className="text-xs text-white/40 uppercase">Instrucciones al Alta</span>
                ...
                {(hospitalization as any).discharge_medications && (
                  <div>
                    <span className="text-xs text-white/40 uppercase">Medicamentos al Alta</span>
