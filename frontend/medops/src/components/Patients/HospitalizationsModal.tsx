...
import IcdDiagnosisSearch, { type DiagnosisEntry } from "@/components/Common/IcdDiagnosisSearch";
...
interface Form {
  ...
  daily_notes: string;
  attending_doctor: number | string | null;
  attending_doctor_name: string | null;
  admission_diagnoses: DiagnosisEntry[];
  vital_signs: {
    weight: number | null;
    height: number | null;
...
const DISCHARGE_TYPES = [
  { value: "home", label: "Alta a domicilio" },
  { value: "transfer", label: "Transferencia" },
...
export default function HospitalizationsModal({ open, onClose, onSave, initial, patientId }: Props) {
  ...
  const handleVitalSignsChange = (field: string, value: string) => {
    ...
    setForm((prev) => ({
      ...prev,
      vital_signs: { ...prev.vital_signs, [field]: numValue },
    }));
  };
  ...
  const handleSubmit = () => {
    ...
    if (form.attending_doctor) {
      if (typeof form.attending_doctor === "number") payload.attending_doctor = form.attending_doctor;
      else payload.attending_doctor_name = form.attending_doctor;
    }
        ...
        <div className="p-6 space-y-5">
          ...
          <div className={sectionClass}>
            <div>
              ...
              <input className={inputClass} value={form.ward} onChange={(e) => handleChange("ward", e.target.value)} placeholder="Ej: Medicina Interna, UCI, Pediatría" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>Número de habitación</label>
                <input className={inputClass} value={form.room_number} onChange={(e) => handleChange("room_number", e.target.value)} placeholder="Ej: 301" />
              </div>
              <div>
                <label className={labelClass}>Número de cama</label>
                <input className={inputClass} value={form.bed_number} onChange={(e) => handleChange("bed_number", e.target.value)} placeholder="Ej: A, B, 1" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>Tipo de admisión</label>
                <select className={inputClass} value={form.admission_type} onChange={(e) => handleChange("admission_type", e.target.value)}>
          ...
          <div className={sectionClass}>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="datetime-local" style={{ colorScheme: "dark" }} className={inputClass} value={form.admission_date} onChange={(e) => handleChange("admission_date", e.target.value)} />
              </div>
              <div>
                <label className={labelClass}>Fecha estimada de alta</label>
                <input type="date" style={{ colorScheme: "dark" }} className={inputClass} value={form.expected_discharge_date} onChange={(e) => handleChange("expected_discharge_date", e.target.value)} />
              </div>
            </div>
          </div>
          ...
          <div className={sectionClass}>
            <DoctorSearchInput
              ...
              onManualConfirm={handleManualDoctorConfirm}
              onClear={clearDoctorSelection}
              selectedName={form.attending_doctor_name}
          ...
          <div className={sectionClass}>
            <IcdDiagnosisSearch
              ...
              diagnoses={form.admission_diagnoses}
              onRemoveDiagnosis={removeDiagnosis}
              onClearAll={() => setForm((prev) => ({ ...prev, admission_diagnoses: [] }))}
            />
          </div>
          ...
          <div className={sectionClass}>
            <div>
              ...
              <textarea className={`${inputClass} min-h-[60px] resize-none`} value={form.chief_complaint} onChange={(e) => handleChange("chief_complaint", e.target.value)} placeholder="Motivo de la hospitalización..." />
            </div>
            <div>
              <label className={labelClass}>Resumen clínico</label>
              <textarea className={`${inputClass} min-h-[80px] resize-none`} value={form.clinical_summary} onChange={(e) => handleChange("clinical_summary", e.target.value)} placeholder="Resumen del estado clínico del paciente..." />
            </div>
            <div>
              <label className={labelClass}>Alergias al ingreso</label>
              <textarea className={`${inputClass} min-h-[50px] resize-none`} value={form.allergies_at_admission} onChange={(e) => handleChange("allergies_at_admission", e.target.value)} placeholder="Alergias conocidas al momento del ingreso..." />
            </div>
          </div>
          ...
          <div className={`${sectionClass} border-t border-white/10 pt-5`}>
            <div>
              ...
              <textarea className={`${inputClass} min-h-[100px] resize-none`} value={form.daily_notes} onChange={(e) => handleChange("daily_notes", e.target.value)} placeholder="Notas diarias de evolución del paciente..." />
            </div>
          </div>
          ...
          <div className={sectionClass}>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input type="number" step="0.1" className={inputClass} value={form.vital_signs.weight ?? ""} onChange={(e) => handleVitalSignsChange("weight", e.target.value)} placeholder="0.0" />
              </div>
              <div>
                <label className={labelClass}>Altura (cm)</label>
                <input type="number" className={inputClass} value={form.vital_signs.height ?? ""} onChange={(e) => handleVitalSignsChange("height", e.target.value)} placeholder="0" />
              </div>
              <div>
                <label className={labelClass}>Temperatura (°C)</label>
                <input type="number" step="0.1" className={inputClass} value={form.vital_signs.temperature ?? ""} onChange={(e) => handleVitalSignsChange("temperature", e.target.value)} placeholder="36.5" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input type="number" className={inputClass} value={form.vital_signs.bp_systolic ?? ""} onChange={(e) => handleVitalSignsChange("bp_systolic", e.target.value)} placeholder="120" />
                </div>
                <div>
                  <label className={labelClass}>PA Diastólica</label>
                  <input type="number" className={inputClass} value={form.vital_signs.bp_diastolic ?? ""} onChange={(e) => handleVitalSignsChange("bp_diastolic", e.target.value)} placeholder="80" />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className={labelClass}>FC (bpm)</label>
                  <input type="number" className={inputClass} value={form.vital_signs.heart_rate ?? ""} onChange={(e) => handleVitalSignsChange("heart_rate", e.target.value)} placeholder="72" />
                </div>
                <div>
                  <label className={labelClass}>FR (/min)</label>
                  <input type="number" className={inputClass} value={form.vital_signs.respiratory_rate ?? ""} onChange={(e) => handleVitalSignsChange("respiratory_rate", e.target.value)} placeholder="16" />
                </div>
                <div>
                  <label className={labelClass}>Sat O2 (%)</label>
                  <input type="number" className={inputClass} value={form.vital_signs.oxygen_saturation ?? ""} onChange={(e) => handleVitalSignsChange("oxygen_saturation", e.target.value)} placeholder="98" />
                </div>
              </div>
            </div>
            {calculateBMI() && (
          ...
          <div className={sectionClass}>
            <div>
              ...
              <textarea className={`${inputClass} min-h-[80px] resize-none`} value={form.complications} onChange={(e) => handleChange("complications", e.target.value)} placeholder="Complicaciones durante la hospitalización..." />
            </div>
          </div>
          ...
          <div className={sectionClass}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select className={inputClass} value={form.discharge_type ?? ""} onChange={(e) => handleChange("discharge_type", e.target.value || null)}>
              ...
              <div>
                ...
                <input type="date" style={{ colorScheme: "dark" }} className={inputClass} value={form.actual_discharge_date ?? ""} onChange={(e) => handleChange("actual_discharge_date", e.target.value || null)} />
              </div>
            </div>
            <div className="mt-4">
              <label className={labelClass}>Resumen de alta</label>
              <textarea className={`${inputClass} min-h-[80px] resize-none`} value={form.discharge_summary ?? ""} onChange={(e) => handleChange("discharge_summary", e.target.value || null)} placeholder="Resumen del proceso de hospitalización..." />
            </div>
            <div className="mt-4">
              <label className={labelClass}>Instrucciones al alta</label>
              <textarea className={`${inputClass} min-h-[60px] resize-none`} value={form.discharge_instructions ?? ""} onChange={(e) => handleChange("discharge_instructions", e.target.value || null)} placeholder="Instrucciones para el paciente al alta..." />
            </div>
            <div className="mt-4">
              <label className={labelClass}>Medicamentos al alta</label>
              <textarea className={`${inputClass} min-h-[60px] resize-none`} value={form.discharge_medications ?? ""} onChange={(e) => handleChange("discharge_medications", e.target.value || null)} placeholder="Medicamentos prescritos al alta..." />
            </div>
          </div>
        </div>
