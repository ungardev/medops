  ...
  return (
    <div className="w-full flex items-center justify-between h-full bg-white/5 px-5 lg:px-6 border-b border-white/10 relative z-[110]">
      <div className="flex items-center gap-4 lg:gap-6 flex-1 min-w-0">
        ...
        <form onSubmit={handleSearchSubmit} className="relative w-full max-w-xl group flex items-center min-w-0">
          ...
          <div className="absolute inset-y-0 right-4 hidden lg:flex items-center">
            <span className="text-xs font-medium text-white/40 border border-white/10 px-2 py-1 rounded-md">Búsqueda Global</span>
          </div>
        </form>
      </div>
      ...
      <div className="flex items-center gap-2 lg:gap-3 shrink-0">
        <div className="relative" ref={notifRef}>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-96 bg-[#1a1a1b] border border-white/15 rounded-xl shadow-2xl z-[120] overflow-hidden animate-in fade-in slide-in-from-top-2">
              <div className="px-5 py-3.5 bg-white/5 border-b border-white/15 flex justify-between items-center">
                <span className="text-sm font-semibold text-white">Notificaciones</span>
                <span className="text-xs text-emerald-400 font-medium px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-md">
              ...
              <ul className="max-h-96 overflow-y-auto divide-y divide-white/5">
                {isLoading ? (
                  <li className="p-10 text-center text-xs text-white/40 animate-pulse">Cargando notificaciones...</li>
                ) : notifications.length === 0 ? (
                  <li className="p-10 text-center text-xs text-white/40">Sin notificaciones</li>
                ) : (
                  notifications.map((n: any) => (
                    ...
                    >
                      <div className="flex gap-3">
                        <div className="mt-0.5 p-2 bg-white/5 rounded-lg text-emerald-400 opacity-80 group-hover/item:opacity-100 transition-opacity">
                          {notificationIcon(n.category)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start">
                            <span className="text-sm font-medium text-white/80 group-hover/item:text-emerald-400 transition-colors">{n.title}</span>
                            <span className="text-xs text-white/40 ml-2 shrink-0">{moment(n.timestamp).fromNow()}</span>
                          </div>
                          <p className="text-xs text-white/50 leading-relaxed mt-1.5 line-clamp-2">{n.description}</p>
                        </div>
                      </div>
                    </li>
                  ))
        ...
        <div className="relative" ref={userMenuRef}>
          <button onClick={() => setMenuOpen(!menuOpen)} className={`flex items-center gap-2 lg:gap-3 p-1.5 lg:p-2 rounded-xl transition-all border ${menuOpen ? 'bg-white/5 border-white/20' : 'border-transparent hover:bg-white/5'}`}>
            <div className="relative">
              <div className="w-10 h-10 bg-white/5 rounded-full flex items-center justify-center border border-white/15">
                {doctorLoading ? (
              ...
              <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-emerald-400 rounded-full border-2 border-[#0a0a0b]"></span>
            </div>
          ...
          {menuOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-[#1a1a1b] border border-white/15 rounded-xl shadow-2xl z-[120] p-2 animate-in fade-in zoom-in-95">
              <button onClick={() => { navigate("/settings/config"); setMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-white/70 hover:text-white hover:bg-white/5 rounded-lg transition-all font-medium group">
                <Settings size={18} className="group-hover:rotate-45 transition-transform" /> Configuración
              </button>
              <div className="h-[1px] bg-white/10 my-2 mx-3"></div>
              <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-red-400/70 hover:bg-red-500/10 rounded-lg transition-all font-medium">
