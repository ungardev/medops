...
const ServiceStatusFilters = ({ categoryType, activeFilter, onFilterChange }: Props) => {
  ...
  return (
    <div className="flex items-center gap-2 bg-white/5 border border-white/15 p-1.5 rounded-xl max-w-full">
      {config.states.map((state: string) => (
        <button
          key={state}
          onClick={() => onFilterChange(state)}
          className={`
            flex-1 min-w-0 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg transition-all duration-200
            ${activeFilter === state 
