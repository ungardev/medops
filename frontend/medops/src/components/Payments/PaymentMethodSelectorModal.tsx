...
export default function PaymentMethodSelectorModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[#1a1a1b] border border-white/15 w-full max-w-lg rounded-xl overflow-hidden">
        ...
        <div className="p-5 space-y-3">
          {PAYMENT_METHODS.map((method) => (
            <button
              ...
              className={`w-full flex items-center gap-4 p-5 border transition-all text-left rounded-xl ${
        ...
        <div className="px-6 py-5 border-t border-white/15 bg-white/5 flex flex-col gap-3">
          <button
            onClick={handleContinue}
            disabled={!selectedMethod}
            className="w-full py-3 bg-emerald-500/15 text-emerald-400 text-sm font-medium hover:bg-emerald-500/25 transition-all disabled:opacity-30 disabled:cursor-not-allowed rounded-xl border border-emerald-500/25"
          >
            Continuar
          </button>
          <button
            onClick={onClose}
            className="w-full py-3 bg-transparent text-white/40 text-sm font-medium hover:text-white/60 transition-all rounded-xl hover:bg-white/5"
          >
            Cancelar
          </button>
        </div>
