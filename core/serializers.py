    ...
    def validate_onset_date(self, value):
        ...
        if value and value > timezone.now().date():
            raise serializers.ValidationError(
                "La fecha de aparición no puede ser futura."
            )
        return value
    ...
    def validate(self, attrs):
        ...
        if status == "active":
            queryset = MedicalHistory.objects.filter(
                patient=patient, condition__iexact=condition, status="active"
            )
...
class ParishSerializer(serializers.ModelSerializer):
    ...
    municipality_id = serializers.PrimaryKeyRelatedField(
        queryset=Municipality.objects.all(), write_only=True, source="municipality"
    )
...
class NeighborhoodSerializer(serializers.ModelSerializer):
    ...
    parish_id = serializers.PrimaryKeyRelatedField(
        queryset=Parish.objects.all(), write_only=True, source="parish"
    )
...
class PatientWriteSerializer(serializers.ModelSerializer):
    ...
    genetic_predispositions = serializers.PrimaryKeyRelatedField(
        queryset=GeneticPredisposition.objects.all(), many=True, required=False
    )
...
class PatientReadSerializer(serializers.ModelSerializer):
    ...
    def get_alerts(self, obj) -> List[Dict[str, Any]]:
        if not hasattr(obj, "alerts"):
            return []
        return [
            {"type": alert.type, "message": alert.message} for alert in obj.alerts.all()
        ]
        ...
        with transaction.atomic():
            ...
            if components_data is not None:
                ...
                for comp_data in components_data:
                    PrescriptionComponent.objects.create(
                        prescription=instance, **comp_data
                    )
...
class PaymentWriteSerializer(serializers.ModelSerializer):
    ...
    def validate_method(self, value):
        ...
        if value not in valid_methods:
            raise serializers.ValidationError(
                f"Método inválido. Opciones: {valid_methods}"
            )
        return value
...
class EventSerializer(serializers.ModelSerializer):
    ...
    def get_description(self, obj):
        # === CHARGE ORDER ===
        if obj.entity == "ChargeOrder" and obj.action in [
            "void",
            "void_by_appointment_cancel",
            "waive",
        ]:
            reason = ""
            actor = ""
        ...
        if obj.entity == "Payment":
            ...
            return (
                f"Monto: ${amount}" if amount else f"Pago #{obj.entity_id} registrado"
            )
        ...
        if obj.entity == "Patient" and obj.action == "create":
            if obj.metadata:
                ...
                return (
                    f"Nombre: {full_name}" if full_name else "Nuevo paciente registrado"
                )
...
class InstitutionSettingsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {"gateway_api_secret": {"write_only": True}}
    ...
    def to_representation(self, instance):
        ...
        # Si existe logo_url (R2), usar directamente
        if instance.logo_url:
            response["logo"] = instance.logo_url
        # Si no, usar logo local si existe
        elif instance.logo:
...
class WaitingRoomEntrySerializer(serializers.ModelSerializer):
    ...
    institution_data = InstitutionSettingsSerializer(
        source="institution", read_only=True
    )
...
class WaitingRoomEntryWriteSerializer(serializers.ModelSerializer):
    institution = serializers.PrimaryKeyRelatedField(
        queryset=InstitutionSettings.objects.all(), required=True
    )
    ...
    def validate(self, attrs):
        ...
        if WaitingRoomEntry.objects.filter(
            patient=patient, institution=institution, status="waiting"
        ).exists():
            raise serializers.ValidationError(
...
class ChargeItemSerializer(serializers.ModelSerializer):
    ...
    doctor_service_name = serializers.CharField(
        source="doctor_service.name", read_only=True
    )
...
class VitalSignsSerializer(serializers.ModelSerializer):
    ...
    def get_vitals_status(self, obj):
        ...
        Ej: 'high', 'low', 'normal'.
        """
        status = {"bp": "normal", "temp": "normal", "o2": "normal"}
...
class AppointmentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not doctor:
            raise serializers.ValidationError(
                {"doctor": "El campo 'doctor' es requerido."}
            )
    ...
    def create(self, validated_data):
        ...
        if validated_data.get("status") in ["tentative", "pending"]:
            if not validated_data.get("tentative_date"):
                validated_data["tentative_date"] = validated_data.get(
                    "appointment_date"
                )
                ...
                if doctor_service_id:
                    ...
                    try:
                        ...
                        doctor_service = DoctorService.objects.filter(
                            id=service_id_int, is_active=True
                        ).first()
                    except Exception:
                        pass
                    ...
                    if doctor_service:
                        ...
                        description = service.get(
                            "description", f"Servicio {doctor_service_id}"
                        )
                        unit_price = Decimal(
                            str(
                                service.get(
                                    "unit_price",
                                    service.get("price_usd", service.get("price", 0)),
                                )
                            )
                        )
            ...
            if initial_payment and total_amount > 0:
                ...
                if payment_amount > 0:
                    payment = Payment.objects.create(
                        ...
                        reference_number=initial_payment.get(
                            "reference_number",
                            f"PRE-{charge_order.id}-{timezone.now().strftime('%Y%m%d%H%M%S')}",
                        ),
                        status="confirmed",
                    )
                    charge_order.recalc_totals()
    ...
    def update(self, instance, validated_data):
        ...
        if services_data is not None:
            charge_order = (
                ChargeOrder.objects.filter(appointment=instance)
                .exclude(status="void")
                .order_by("-issued_at")
                .first()
            )
...
class ChargeOrderPaymentSerializer(serializers.ModelSerializer):
    ...
    appointment_date = serializers.DateTimeField(
        read_only=True
    )  # ahora toma el campo anotado en el queryset
    total_amount = serializers.FloatField(
        source="total", read_only=True
    )  # homogéneo con FloatField
...
class DoctorOperatorSerializer(serializers.ModelSerializer):
    ...
    active_institution = serializers.PrimaryKeyRelatedField(
        queryset=InstitutionSettings.objects.all(), required=False, allow_null=True
    )
    ...
    def to_representation(self, instance):
        ...
        rep["institution_ids"] = list(
            instance.institutions.values_list("id", flat=True)
        )
        return rep
...
class DashboardSummarySerializer(serializers.Serializer):
    ...
    )  # ✅ citas con actividad clínica real
    ...
    total_events = (
        serializers.IntegerField()
    )  # eventos críticos genéricos (mantener para auditoría)
    total_canceled_orders = (
        serializers.IntegerField()
    )  # ✅ nuevo: solo órdenes anuladas en el rango
...
class MedicalTestSerializer(serializers.ModelSerializer):
    ...
    def get_display_name(self, obj):
        return obj.test_name_override or (
            obj.catalog_item.name if obj.catalog_item else obj.get_test_type_display()
        )
...
class AppointmentDetailSerializer(AppointmentSerializer):
    ...
    def get_charge_order(self, obj):
        ...
        """
        order = (
            obj.charge_orders.exclude(status__in=["void", "waived"])
            .order_by(
                models.Case(
...
class FamilyHistorySerializer(serializers.ModelSerializer):
    ...
    relative_display = serializers.CharField(
        source="get_relative_display", read_only=True
    )
...
class HabitSerializer(serializers.ModelSerializer):
    # ✅ Display para mostrar etiquetas legibles
    type_display = serializers.CharField(source="get_type_display", read_only=True)
    smokes_currently_display = serializers.CharField(
        source="get_smokes_currently_display", read_only=True
    )
    tobacco_type_display = serializers.CharField(
        source="get_tobacco_type_display", read_only=True
    )
    smoking_frequency_display = serializers.CharField(
        source="get_smoking_frequency_display", read_only=True
    )
    drinks_alcohol_display = serializers.CharField(
        source="get_drinks_alcohol_display", read_only=True
    )
    alcohol_frequency_display = serializers.CharField(
        source="get_alcohol_frequency_display", read_only=True
    )
    alcohol_quantity_display = serializers.CharField(
        source="get_alcohol_quantity_display", read_only=True
    )
    binge_frequency_display = serializers.CharField(
        source="get_binge_frequency_display", read_only=True
    )
    exercise_frequency_display = serializers.CharField(
        source="get_exercise_frequency_display", read_only=True
    )
    exercise_intensity_display = serializers.CharField(
        source="get_exercise_intensity_display", read_only=True
    )
    diet_type_display = serializers.CharField(
        source="get_diet_type_display", read_only=True
    )
    sleep_quality_display = serializers.CharField(
        source="get_sleep_quality_display", read_only=True
    )
    uses_drugs_display = serializers.CharField(
        source="get_uses_drugs_display", read_only=True
    )
...
class PatientClinicalProfileSerializer(serializers.ModelSerializer):
    ...
    def get_alerts(self, obj):
        ...
        return [
            ...
            for a in obj.alerts.filter(is_active=True)
        ]
        ...
        if n:
            parts.extend(
                [
                    n.name,
                    n.parish.name,
                    n.parish.municipality.name,
                    n.parish.municipality.state.name,
                ]
            )
...
class DoctorPaymentConfigSerializer(serializers.ModelSerializer):
    ...
    enabled_methods_details = PaymentGatewayListSerializer(
        source="enabled_methods", many=True, read_only=True
    )
...
class SurgerySerializer(serializers.ModelSerializer):
    ...
    anesthesiologist_name = serializers.CharField(
        source="anesthesiologist.full_name", read_only=True, allow_null=True
    )
    specialty_name = serializers.CharField(
        source="specialty.name", read_only=True, allow_null=True
    )
    diagnosis_icd = serializers.CharField(
        source="diagnosis.icd_code", read_only=True, allow_null=True
    )
    diagnosis_title = serializers.CharField(
        source="diagnosis.title", read_only=True, allow_null=True
    )
    status_display = serializers.CharField(source="get_status_display", read_only=True)
    surgery_type_display = serializers.CharField(
        source="get_surgery_type_display", read_only=True
    )
...
class SurgeryCreateSerializer(serializers.ModelSerializer):
    ...
    surgical_assistants_name = serializers.CharField(
        write_only=True, required=False, allow_null=True
    )
    ...
    def create(self, validated_data):
        surgeon_name = validated_data.pop("surgeon_name", None)
        anesthesiologist_name = validated_data.pop("anesthesiologist_name", None)
        surgical_assistants_ids = validated_data.pop("surgical_assistants", None)
        surgical_assistants_name = validated_data.pop("surgical_assistants_name", None)
        diagnoses = validated_data.pop("diagnoses", None)

        if surgeon_name and not validated_data.get("surgeon"):
            doctor = DoctorOperator.objects.filter(
                full_name__iexact=surgeon_name
            ).first()
            if doctor:
                validated_data["surgeon"] = doctor

        if anesthesiologist_name and not validated_data.get("anesthesiologist"):
            doctor = DoctorOperator.objects.filter(
                full_name__iexact=anesthesiologist_name
            ).first()
            if doctor:
        ...
        if surgical_assistants_ids:
            ...
            surgery.assistants.set(assistants)
        elif surgical_assistants_name:
            assistant_names = [n.strip() for n in surgical_assistants_name.split(",")]
            assistants = DoctorOperator.objects.filter(full_name__in=assistant_names)
            surgery.assistants.set(assistants)

        if diagnoses and len(diagnoses) > 0 and not validated_data.get("diagnosis"):
            first_diag = diagnoses[0]
            diag_id = first_diag.get("id")
            if diag_id:
                try:
                    diagnosis = Diagnosis.objects.get(id=diag_id)
...
class HospitalizationSerializer(serializers.ModelSerializer):
    ...
    admission_diagnosis_icd = serializers.CharField(
        source="admission_diagnosis.icd_code", read_only=True, allow_null=True
    )
    admission_diagnosis_title = serializers.CharField(
        source="admission_diagnosis.title", read_only=True, allow_null=True
    )
    status_display = serializers.CharField(source="get_status_display", read_only=True)
    ...
    discharge_type_display = serializers.CharField(
        source="get_discharge_type_display", read_only=True, allow_null=True
    )
    institution_name = serializers.CharField(
        source="institution.name", read_only=True, allow_null=True
    )
    ...
    class Meta:
        ...
        fields = [
            ...
            "admission_diagnosis",
            "admission_diagnosis_icd",
            "admission_diagnosis_title",
            "admission_type",
...
class HospitalizationCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        attending_doctor_name = validated_data.pop("attending_doctor_name", None)
        diagnoses = validated_data.pop("diagnoses", None)

        if attending_doctor_name and not validated_data.get("attending_doctor"):
            doctor = DoctorOperator.objects.filter(
                full_name__iexact=attending_doctor_name
            ).first()
            if doctor:
        ...
        if diagnoses and len(diagnoses) > 0:
            ...
            if diag_id:
                try:
                    diagnosis = Diagnosis.objects.get(id=diag_id)
                    hospitalization.admission_diagnosis = diagnosis
                    hospitalization.save()
                except Diagnosis.DoesNotExist:
                    pass
