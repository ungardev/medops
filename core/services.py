...
from .serializers import (
    ...
    ICD11EntrySerializer,
    DiagnosisWriteSerializer,
    MedicalTestSerializer,
    MedicalReferralSerializer,
    PrescriptionWriteSerializer,
    TreatmentWriteSerializer,
    MedicalTestWriteSerializer,
    MedicalReferralWriteSerializer,
    SpecialtySerializer,
    PersonalHistorySerializer,
    ...
    PatientClinicalProfileSerializer,
    AllergySerializer,
...
def get_bcv_rate_logic():
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto(
                "https://www.bcv.org.ve/", wait_until="networkidle", timeout=30000
            )
        ...
        if raw_val:
            try:
                ...
                BCVRateCache.objects.update_or_create(
                    date=today, defaults={"value": rate_decimal}
                )
...
def get_dashboard_summary_data(
    start_date=None, end_date=None, range_param=None, currency="USD", status_param=None
):
    """
    ...
    confirmed_amount = Payment.objects.filter(status="confirmed").aggregate(
        s=Sum("amount")
    ).get("s") or Decimal("0")
    ...
    appt_trend = list(
        Appointment.objects.filter(
            appointment_date__range=(start, end), status="completed"
        )
    ...
    pay_trend = list(
        Payment.objects.filter(
            status="confirmed", received_at__date__range=(start, end)
        )
    ...
    return {
        ...
        "waiting_room_count": WaitingRoomEntry.objects.filter(
            arrival_time__date__range=(start, end), status="waiting"
        ).count(),
        "total_payments_amount": convert(confirmed_amount),
        "financial_balance": convert(max(total_amount - balance_due, Decimal("0"))),
        "appointments_trend": [
            {"date": str(r["date"]), "value": int(r["value"])} for r in appt_trend
        ],
        "payments_trend": [
            {"date": str(r["date"]), "value": float(r["value"])} for r in pay_trend
        ],
        "bcv_rate": {
            "value": rate_val,
            "unit": "VES_per_USD",
...
def get_daily_appointments() -> List[Dict[str, Any]]:
    ...
    return cast(
        List[Dict[str, Any]], AppointmentSerializer(appointments, many=True).data
    )
...
def export_institutional_report(
    ...
    if export_format == "pdf":
        ...
        if inst:
            ...
            elements.append(
                Paragraph(f"Dirección: {inst.address or ''}", styles["Normal"])
            )
            elements.append(
                Paragraph(
                    f"Tel: {inst.phone or ''} • RIF: {inst.tax_id or ''}",
                    styles["Normal"],
                )
            )
        ...
        elements.append(
            Paragraph(
                f"Fecha: {timezone.now().strftime('%Y-%m-%d %H:%M')}", styles["Normal"]
            )
        )
    ...
    # --- LÓGICA EXCEL ---
        ...
        for r in data_serialized:
            ...
            amount_dec = Decimal(str(r.get("amount") or "0"))
            amount_val = float(
                (amount_dec * rate).quantize(Decimal("0.01"), ROUND_HALF_UP)
            )
...
def generate_pdf_document(category: str, queryset, appointment):
    ...
    if category == "prescription":
        ...
        def route_label(val):
            return dict(ROUTE_CHOICES).get(val, val)

        def freq_label(val):
            return dict(FREQUENCY_CHOICES).get(val, val)

        def unit_label(val):
            return dict(UNIT_CHOICES).get(val, val)

        for p in queryset:
            med_name = ""
            if getattr(p, "medication_catalog", None):
                med_name = getattr(p.medication_catalog, "name", "") or getattr(
                    p.medication_catalog, "title", ""
                )
            if not med_name:
        ...
        for t in queryset:
            items.append(
                {
                    "description": safe(getattr(t, "plan", None))
                    or safe(getattr(t, "description", None)),
        ...
        for r in queryset:
            spec_names = (
                [s.name for s in r.specialties.all()]
                if hasattr(r, "specialties")
                else []
            )
            items.append(
                {
                    ...
                    "urgency": getattr(
                        r, "get_urgency_display", lambda: safe(r.urgency)
                    )(),
                    "status": getattr(
                        r, "get_status_display", lambda: safe(r.status)
                    )(),
    ...
    return ContentFile(
        pdf_bytes or b"", name=f"{category}_{appointment.id}.pdf"
    ), audit_code
...
def get_audit_logic(
    ...
    if patient_id:
        ...
        patient_event_ids = list(
            Event.objects.filter(
                entity="Patient", entity_id=patient_id_int
        ...
        appointment_ids = list(
            Appointment.objects.filter(patient_id=patient_id_int).values_list(
                "id", flat=True
            )
        )
        ...
        if appointment_ids:
            # Citas directas
            appointment_event_ids = list(
                Event.objects.filter(
                    entity="Appointment", entity_id__in=appointment_ids
                ).values_list("id", flat=True)
            )
            related_entity_ids.extend(appointment_event_ids)

            # C) Órdenes de cargo del paciente
            charge_order_ids = list(
                ChargeOrder.objects.filter(patient_id=patient_id_int).values_list(
                    "id", flat=True
                )
            )

            if charge_order_ids:
                charge_order_event_ids = list(
                    Event.objects.filter(
                        entity="ChargeOrder", entity_id__in=charge_order_ids
                    ).values_list("id", flat=True)
                )
                ...
                if payment_ids:
                    payment_event_ids = list(
                        Event.objects.filter(
                            entity="Payment", entity_id__in=payment_ids
                        ).values_list("id", flat=True)
                    )
                    related_entity_ids.extend(payment_event_ids)

            # E) Entradas en sala de espera del paciente
            waiting_room_ids = list(
                WaitingRoomEntry.objects.filter(patient_id=patient_id_int).values_list(
                    "id", flat=True
                )
            )

            if waiting_room_ids:
                waiting_event_ids = list(
                    Event.objects.filter(
                        entity="WaitingRoomEntry", entity_id__in=waiting_room_ids
                    ).values_list("id", flat=True)
                )
    ...
    if split_by_category:
        ...
        general = qs.filter(entity__in=["Patient", "Appointment", "WaitingRoomEntry"])[
            :lim
        ]
    ...
    if filters and filters.get("dashboard_stats"):
        return {
            ...
            "by_entity": list(
                Event.objects.values("entity").annotate(total=Count("id"))
            ),
            "by_action": list(
                Event.objects.values("action").annotate(total=Count("id"))
            ),
        }
...
def get_pending_appointments():
    ...
    for appt in appointments:
        ...
        total_paid = sum(
            float(p.amount or 0) for p in appt.payments.all() if p.status == "confirmed"
        )
...
def get_report_data(
    report_type: str, start: Optional[date], end: Optional[date]
) -> List[Dict[str, Any]]:
...
def build_chargeorder_pdf(charge_order, user) -> Tuple[bytes, str, str]:
    ...
    # Generar QR
    qr_payload = (
        f"Consulta:{appointment.id}|ChargeOrder:{charge_order.id}|Audit:{audit_code}"
    )
    qr_img = qrcode.make(qr_payload)
    buffer = BytesIO()
    qr_img.save(buffer, "PNG")
...
def check_patient_safety(patient_id: int) -> List[Dict[str, Any]]:
    ...
    if allergies.exists():
        ...
        alerts.append(
            {"type": "danger", "message": msg, "icon": "ExclamationTriangleIcon"}
        )
    ...
    last_vitals = (
        VitalSigns.objects.filter(patient=patient).order_by("-created_at").first()
    )
...
def lock_consultation_integrity(appointment_id: int):
    ...
    with transaction.atomic():
        ...
        if not note.is_locked:
            ...
            content_block = (
                f"{note.subjective}{note.objective}{note.analysis}{note.plan}"
            )
...
def generate_generic_pdf(
    instance: Any, category: str, institution: InstitutionSettings
) -> Tuple[bytes, str, str]:
    """
    ...
    def calculate_age(birth_date):
        ...
        try:
            birth = (
                ...
                else datetime.strptime(str(birth_date), "%Y-%m-%d").date()
            )
            age = (
                today.year
                - birth.year
    ...
    if category == "medical_referral":
        ...
        if hasattr(instance, "referred_to_doctor") and instance.referred_to_doctor:
            ...
            hasattr(instance, "referred_to_external") and instance.referred_to_external
        ):
        ...
        context = {
            ...
            "referred_to_institution": getattr(
                instance, "referred_to_institution", None
            )
            or "",
            ...
            "instructions": getattr(instance, "clinical_summary", None)
            or getattr(instance, "instructions", None)
            or "",
        }
        ...
        items_data.append(
            {
                "description": getattr(instance, "plan", None)
        ...
        test_type_display = (
            instance.get_test_type_display()
            if hasattr(instance, "get_test_type_display")
            else getattr(instance, "test_type", "")
        )
...
def generate_prescription_bundle(
    prescriptions: List[Any], appointment
) -> Tuple[bytes, str, str]:
    """
    ...
    for pres in prescriptions:
        ...
        if hasattr(pres, "diagnosis") and pres.diagnosis:
            ...
            diagnosis_title = (
                getattr(pres.diagnosis, "title", "")
                or getattr(pres.diagnosis, "description", "")
                or ""
            )
        ...
        # Formatear campos
        route = (
            pres.get_route_display()
            if hasattr(pres, "get_route_display")
            else getattr(pres, "route", "") or ""
        )
        frequency = (
            pres.get_frequency_display()
            if hasattr(pres, "get_frequency_display")
            else getattr(pres, "frequency", "") or ""
        )
...
def generate_treatment_bundle(
    treatments: List[Any], appointment
) -> Tuple[bytes, str, str]:
    """
...
def bulk_generate_appointment_docs(appointment, user, request=None) -> Dict[str, Any]:
    ...
    prescriptions = (
        Prescription.objects.filter(diagnosis__appointment=appointment)
    ...
    if prescriptions.exists():
        try:
            ...
            if len(prescription_list) == 1:
                ...
                pdf_bytes, filename, audit_code = generate_generic_pdf(
                    prescription_list[0], "prescription", appointment.institution
                )
                ...
                pdf_bytes, filename, audit_code = generate_prescription_bundle(
                    prescription_list, appointment
                )
                description = (
                    f"Recetas Consolidadas ({len(prescription_list)} medicamentos)"
                )
            ...
            errors.append(
                {"category": "prescription", "item_id": None, "error": str(e)}
            )
    ...
    if treatments.exists():
        try:
            ...
            if len(treatment_list) == 1:
                ...
                pdf_bytes, filename, audit_code = generate_generic_pdf(
                    treatment_list[0], "treatment", appointment.institution
                )
                title = (
                    treatment_list[0].plan or treatment_list[0].title or "Tratamiento"
                )
                description = f"Tratamiento: {title[:100]}"
            else:
                # ✅ NUEVO: Generar documento consolidado
                pdf_bytes, filename, audit_code = generate_treatment_bundle(
                    treatment_list, appointment
                )
                description = (
                    f"Tratamientos Consolidados ({len(treatment_list)} planes)"
                )
            ...
            errors.append({"category": "treatment", "item_id": None, "error": str(e)})

    # ✅ Mantener otros generadores (referencias, exámenes) - sin cambios
    other_generators = {
        "medical_referral": appointment.referrals.all()
        .select_related("diagnosis")
    ...
    for category, queryset in other_generators.items():
        for item in queryset:
            try:
                pdf_bytes, filename, audit_code = generate_generic_pdf(
                    item, category, appointment.institution
                )

                # Descripciones específicas por categoría
                if category == "medical_referral":
                    specialties = (
                        [s.name for s in item.specialties.all()[:3]]
                        if hasattr(item, "specialties")
                        else []
                    )
                    referred_to = (
                        item.referred_to_doctor.full_name
                        if hasattr(item, "referred_to_doctor")
                        and item.referred_to_doctor
                        else ""
                    )
                    description = (
                        f"Referencia: {', '.join(specialties)}"
                        if specialties
                        else f"Referencia a: {referred_to}"
                        if referred_to
                        else "Referencia Médica"
                    )
                else:
                    test_type = (
                        item.get_test_type_display()
                        if hasattr(item, "get_test_type_display")
                        else getattr(item, "test_type", "Examen")
                    )
...
def get_advanced_metrics() -> Dict[str, Any]:
    ...
    current_revenue = (
        Payment.objects.filter(received_at__date=today).aggregate(Sum("amount"))[
            "amount__sum"
        ]
        or 0
    )
    ...
    growth = (
        ((current_patients - prev_patients) / prev_patients * 100)
        if prev_patients > 0
        else 100
    )
    ...
    return {
        ...
        "appointmentVolume": list(
            Appointment.objects.filter(
                appointment_date__range=[today - timedelta(days=7), today]
            )
...
def get_institution_settings(request=None, active_only=False):
    ...
    try:
        ...
        # RESILIENCIA: Si doctor_profile no se resolvió via lazy loading,
        # hacer consulta directa (recupera contexto JWT que pudo no cargarse)
        if (
            not doctor
            and request
        ...
        # RESILIENCIA: Si el doctor existe pero no tiene instituciones asignadas,
        # usar active_institution como fallback directo
        if not institutions.exists() and doctor.active_institution:
            institutions = InstitutionSettings.objects.filter(
...
def get_doctor_config(
    ...
    logger.info(
        f"[GET_DOCTOR_CONFIG] is_authenticated={is_auth}, user={request.user if request else None}"
    )

    if is_auth and request:
        doctor = getattr(request.user, "doctor_profile", None)
        logger.info(
            f"[GET_DOCTOR_CONFIG] doctor from user.doctor_profile: id={doctor.id if doctor else None}"
        )
        if not doctor:
            return None
    else:
        doctor = DoctorOperator.objects.first()
        logger.info(
            f"[GET_DOCTOR_CONFIG] doctor from fallback first(): id={doctor.id if doctor else None}"
        )
        if not doctor:
            return None
    ...
    try:
        ...
        logger.info(
            f"[GET_DOCTOR_CONFIG] payment_config found: bank_name={payment_config.bank_name}"
        )
...
def update_doctor_config(
    ...
    specialty_ids = (
        data.getlist("specialty_ids")
        if hasattr(data, "getlist")
        else data.get("specialty_ids")
    )
    ...
    if bank_data:
        ...
        payment_config, created = DoctorPaymentConfig.objects.get_or_create(
            doctor=doctor_obj, defaults=bank_data
        )
        if not created:
...
def create_institution_for_doctor(
    data: Dict[str, Any], doctor: DoctorOperator
) -> InstitutionSettings:
    """
...
def add_institution_to_doctor(
    institution_id: int, doctor: DoctorOperator
) -> InstitutionSettings:
    """
...
def delete_institution_from_doctor(
    institution_id: int, doctor: DoctorOperator
) -> Dict[str, Any]:
    """
...
class PaymentOCRService:
                ...
                try:
                    ...
                    for config in configs:
                        text = pytesseract.image_to_string(
                            processed_img, lang="spa+eng", config=config
                        )
    ...
    def _extract_basic(cls, img):
        ...
        try:
            ...
            text = pytesseract.image_to_string(
                img_rgb, lang="spa+eng", config=r"--oem 3 --psm 6"
            )
            text = cls._normalize_ocr_text(text)
            data = cls._parse_payment_text(text)
    ...
    @classmethod
    def _extract_monto(
        cls, text: str, exclude_ref: Optional[str] = None
    ) -> Optional[str]:
